...
fn get_persona_skill_name(character: &CharacterMeta) -> Option<String> {
    ...
    if get_skill_content(&underscore).is_some() {
        return Some(underscore);
    }
    if get_skill_content(&dash).is_some() {
        return Some(dash);
    }
...
fn character_system_with_memory(skill_content: &str, character: &CharacterMeta, core: &str, memory_context: &str, target_time: Option<&str>) -> String {
    let now = Local::now();
...
fn character_system(skill_content: &str, character: &CharacterMeta, core: &str) -> String {
    character_system_with_memory(skill_content, character, core, "", None)
}
...
pub async fn generate_post_text_with_memory(
    ...
    let system = character_system_with_memory(skill, character, &core, &memory_context, target_time);
...
pub async fn generate_comment_text_with_memory(
    ...
    let core = crate::tools::read_core(app);
    let memory_context = super::character_memory::build_memory_context(app, &character.id);
    let system = character_system_with_memory(skill, character, &core, &memory_context, None);
